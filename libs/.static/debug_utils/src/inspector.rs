pub mod plugin {
    ...
    impl Plugin for SwitchableEguiInspectorPlugin {
        fn build(&self, app: &mut App) {
            ...
            app.add_plugins(
                bevy_inspector_egui::quick::WorldInspectorPlugin::new().run_if(inspector_toggled),
            );
            app.add_systems(Update, toggle_inspector);
        }
    }

    fn inspector_toggled(cond: Res<InspectorEnabled>) -> bool {
        return cond.0;
    }

    fn toggle_inspector(mut cond: ResMut<InspectorEnabled>, keyboard: Res<ButtonInput<KeyCode>>) {

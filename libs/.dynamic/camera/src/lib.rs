...
use bevy::{camera::ScalingMode, input::mouse::MouseWheel, prelude::*, render::view::Hdr};
use kaiv_utils::prelude::ExpDecay;
use properties::{MAX_DT, Player};
use room::*;
...
impl Plugin for CameraPlugin {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(Update, tick_camera)
...
fn focus_player(
    ...
    pq: Query<&GlobalTransform, (With<Player>, Without<Camera2d>)>,
    mut cq: Query<(Entity, &mut Projection), (With<Camera2d>, Without<Player>)>,
    mut cmd: Commands,
...
fn tick_camera(
    time: Res<Time>,
    mut camera: Query<(&Camera, &mut Projection, &mut Transform, &GlobalTransform), With<Camera2d>>,
    mut camera_controller: ResMut<CameraController>,   
